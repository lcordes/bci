...
from data_acquisition.preprocessing import preprocess_recording as preprocess_training
from analysis.analysis_competition_IV_2a import preprocess_recording as preprocess_benchmark
from feature_extraction.extractors import CSPExtractor
from classification.classifiers import CLASSIFIERS

def get_align_mat(X):
    n_trials = X.shape[0]
    cov_mats = [np.cov(X[trial, :, :]) for trial in range(n_trials)]
    cov_array = np.stack(cov_mats, axis=0)
    ref_mat = np.mean(cov_array, axis=0)
    return inv(sqrtm(ref_mat))
    
    
def align(X, align_mat):
    n_trials = X.shape[0]
    trials_aligned = [np.matmul(align_mat, X[trial, :, :]) for trial in range(n_trials)]
    X_aligned = np.stack(trials_aligned, axis=0)
    assert X.shape == X_aligned.shape
    return X_aligned
...
def online_simulation(data_set, config, title, align_X=True, oversample=1):
    ...
    train_sizes = range(60, pool_size+step_size, step_size)
    ...
    for u, user in enumerate(users):
        ...
        for rep in range(repetitions):
            ...
            for t_idx, train_size in enumerate(train_sizes):
                ...
                tf_model = train_model(X_train, y_train, config)
                tf_acc = test_model(X_test, y_test, tf_model)
                results[u, rep, t_idx] = tf_acc
...
def plot_online_sim_results(title):
        ...
        x = list(range(4, 44, 4)) if data.shape[1] == 10 else list(range(60,104, 4))
...
def plot_online_sim_comparison(base_results, tf_results):
...
if __name__ == "__main__":
    ...
    title = "Online simulation (benchmark data, 8-30, no assist users, no align, 60-100 trials)"
